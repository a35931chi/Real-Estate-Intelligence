...
htemp = pd.read_sql(hsql_query, engine, index = 'Date')
home_value_data = pd.concat([home_value_data, htemp], axis = 0)
home_value_data.drop_duplicates(inplace = True)

rsql_query = 'select * from home_value;'
rtemp = pd.read_sql(hsql_query, engine, index = 'Date')
rental_value_data = pd.concat([rental_value_data, rtemp], axis = 0)
rental_value_data.drop_duplicates(inplace = True)

print(home_value_data.shape) #(101599, 2) on 2017-07-26
print(rental_value_data.shape) #(49428, 2) on 2017-07-26
